...
export const CertificateStatus = ({certificateData, goBack}) => {
    ...
    function getTestCertificateStatusAsString(data) {

        return `${getTestName(data)} Test Certificate for ${getDiseaseName(data)}`
    ...
    function getTestName(data) {
        ...
        return data["evidence"][0]["testName"]
    }

    function getDiseaseName(data) {
        if (!data || !data["evidence"]) {
            return ""
        }
        return data["evidence"][0]["disease"]
    }
    ...
    return (
        isLoading ? <Loader/> : <div className="certificate-status-wrapper">
            ...
            <br/>
            {
                isValid && <h5>
                    {
                        getTestCertificateStatusAsString(data)
                    }
                </h5>
            }
            {
