...
export const Members = () => {
    ...
    axios
      ...
      .then((res) => {
        const programs = res.data
          .filter((p) => new Date(p.endDate + " 00:00") - new Date() > 0)
          .map((obj) => ({
            name: obj.name,
            id: obj.osid,
            osid: obj.osid,
  ...
  function onDeleteRecipientProgram(recipientOsid, programId) {
    ...
    axios
      .delete(
        `/divoc/api/citizen/recipient/${member.osid}/program/${appointment.programId}`,
        config
      )
      .then((res) => {
        setIsLoading(true);
  ...
  function callCancelAppointment() {
    ...
    axios
      .delete("/divoc/api/citizen/appointment", config)
      .then((res) => {
        setIsLoading(true);
        setTimeout(() => {
          fetchRecipients();
  ...
  function callDeleteRecipient() {
    ...
    axios
      .delete("/divoc/api/citizen/recipients", config)
      .then((res) => {
        setIsLoading(true);
        setTimeout(() => {
          fetchRecipients();
  ...
  return (
    <div className="main-container">
      ...
      <Container fluid>
        ...
        {selectedMemberIndex > -1 && members.length > 0 && showProgramModal && (
          <RegisterProgram
    ...
    <>
      <CustomModal
        ...
        title={
          memberAction === CANCEL_APPOINTMENT
            ? t("registration.cancelAppointment")
            : t("registration.deleteRegistration")
        }
        showModal={true}
        onClose={() => {
          setShowModal(false);
      ...
      >
        <div className="d-flex flex-column ">
          {memberAction === DELETE_REGISTERED_PROGRAM && (
            <spam>
              {t("registration.programDeleteForBeneficiaryInfo", {
                program:
                  programsById[
                    member["appointments"][selectedAppointmentIndex].programId
                  ].name,
          ...
          {memberAction === CANCEL_APPOINTMENT && (
            <>
              ...
              <span className="mt-1">
                {t("entity.enrollmentNumber")}: {member.code}
              </span>
              <span className="mt-1 ">
                At{" "}
                {`${member["appointments"][selectedAppointmentIndex].facilityDetails.facilityName}, ${member["appointments"][selectedAppointmentIndex].facilityDetails.district}, \n ${member["appointments"][selectedAppointmentIndex].facilityDetails.state}, ${member["appointments"][selectedAppointmentIndex].facilityDetails.pincode}`}
              </span>
              <span className="mt-1">
                {formatDate(
                  member["appointments"][selectedAppointmentIndex]
                    .appointmentDate || ""
                )}
                ,{" "}
                {member["appointments"][selectedAppointmentIndex]
...
const MemberCard = (props) => {
      ...
      return (
        <div className="appointment-wrapper">
          <div className="appointment-details">
            <span className="appointment-title text-capitalize">
              {t("entity.program")}
            </span>
            <span className="font-weight-bold">
              {props.programsById[programId]?.name}
            </span>
          </div>
          <div className="appointment-details">
            <span className="appointment-title text-capitalize">
              {t("entity.enrollmentNumber")}
            </span>
          ...
          <div className="appointment-schedule">
            {programWiseAppointments.map((appointment) => (
              <AppointmentTimeline
                ...
                showDeleteRecipientProgram={canShowDeleteRecipientProgram(
                  appointment
                )}
                onDeleteRecipientProgram={() => {
                ...
                showCancelAppointment={isAppointmentCancellationAllowed(
                  appointment
                )}
                onCancelAppointment={() => {
                ...
                isAppointmentScheduled={
                  appointment.enrollmentScopeId !== "" || appointment.certified
                }
  ...
  member.appointments.forEach((appointment) => {
    if (
      !registeredProgramIds.includes(appointment.programId) &&
        ...
        <Card className="member-card">
          ...
          <Accordion.Collapse eventKey={"" + 0}>
            <Card.Body className="member-card-body">
              ...
              {registeredProgramIds.length <
                Object.keys(props.programsById).length && (
                <CustomButton
                  CustomButton
                  isLink
                  onClick={() => props.onRegisterProgram(props.index)}
                  className="appointment-link-btn d-flex align-items-center"
                >
                  <span className="appointment-add-program mr-2">+</span>
                  <span> {t("registration.newProgram")}</span>
                </CustomButton>
              )}
            </Card.Body>
          </Accordion.Collapse>
...
const AppointmentTimeline = ({
  ...
  function onDownloadCertificate() {
    ...
    window
      .open(
        `${certificateURL}/certificate/api/certificate/${certificateId}?authToken=${token}`,
        "_blank"
      )
      .focus();
  }
  function onDownloadQRCode() {
    ...
    window
      .open(
         `${certificateURL}/certificate/api/certificate/QRCode/${certificateId}?authToken=${token}`
        )
      .focus();
  }
      ...
      {!certified && (
        <span
          className={`appointment-line ${
            isAppointmentScheduled && "appointment-line-mini"
          }`}
        />
      )}
      {!isAppointmentScheduled && dose === "1" && (
        <div className="timeline-node" style={{ zIndex: 1 }}>
          <img
            src={CheckImg}
            className="appointment-active-circle"
            alt="check"
          />
          <div className="timeline-node-text">
            <span className="appointment-active-title font-weight-bold">
              {t("entity.registration")}
            </span>
            <span className="appointment-active-title">{registeredDate}</span>
            {showDeleteRecipientProgram && (
              <CustomButton
                isLink
                onClick={onDeleteRecipientProgram}
                className="appointment-link-btn text-capitalize"
              >
        ...
        <div className="timeline-node" style={{ zIndex: 1 }}>
          ...
          <div className="timeline-node-text">
            <span
              className={`${
                isAppointmentScheduled
                  ? "appointment-active-title font-weight-bold"
                  : "appointment-inactive-title"
              } text-capitalize`}
            >
              {t("entity.scheduled")} ({ordinal_suffix_of(dose)}{" "}
              {t("entity.dose")})
            </span>
            {isAppointmentScheduled && (
              <span className="appointment-active-title">
                {formatDate(appointmentDate)} {appointmentSlot}
              </span>
            )}
            {isAppointmentScheduled && (
              <span className="appointment-active-title">
                {facilityDetails.facilityName}, {facilityDetails.district},{" "}
                {facilityDetails.state}, {facilityDetails.pincode}
              </span>
            )}
            {showBookAppointment && (
              <CustomButton
                ...
                className="appointment-link-btn text-capitalize"
              >
                {t("registration.bookAppointment")}
              </CustomButton>
            )}
            {showCancelAppointment && (
              <CustomButton
          ...
          <span
            className={`${
              certified
                ? "appointment-active-title font-weight-bold"
                : "appointment-inactive-title"
            } text-capitalize d-flex flex-wrap`}
          >
            {t("entity.vaccinated")}{" "}
            <span className="ml-1 ml-lg-0">
              {" "}
              ({ordinal_suffix_of(dose)} {t("entity.dose")})
            </span>
            {certified && (
              <span className="appointment-active-title font-weight-normal ml-1">
          ...
          {certified && (
            <>
              <CustomButton
                ...
                className="appointment-link-btn"
              >
...
const RegisterProgram = ({
  ...
  function registerProgram() {
    ...
    axios
      .post(
        `/divoc/api/citizen/recipient/${member.osid}/program/${formData.programId}`,
        data,
        config
      )
      .then((res) => {
  ...
  let programAvailable = programs.filter(
    (program) => !registeredProgramIds.includes(program.osid)
  );
  return (
    <Modal
      size={"xl"}
